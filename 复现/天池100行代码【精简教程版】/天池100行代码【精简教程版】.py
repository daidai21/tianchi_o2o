...
def getDiscountType(row):  # 获取折扣类型
    if pd.isnull(row):
        return np.nan
    elif ':' in row:
        return 1
...
def convertRate(row):  # 转化率
    """Convert discount to rate"""  # 将折扣转化为百分比
    if pd.isnull(row):
        return 1.0
    elif ':' in str(row):
        rows = row.split(':')
...
def getDiscountJian(row):  # 获得折扣Jian
    if ':' in str(row):
        rows = row.split(':')
        return int(rows[1])
    else:
